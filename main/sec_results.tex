...
This figure shows the trend of the observer getting better and better state estimates as a function of the number of sensing locations $\nsamp$. Note that we checked the performance of training a GP with only $1,000$ samples as a control, but the average error was about 10 Kelvins, i.e. much worse than FKO.
 Time required for kernel observer is much lesser than retraining the model every time step, see Figures %13(g)-13(h). 
...
As a proof-of-concept in applying our methods for learning dynamics and/or inferring the state of spatiotemporally-evolving systems, we consider the problem of predicting weed growth in agricultural fields.
...
\rr{
\subsection{$k$-Invariant Subspaces Clustering}
First we tested our algorithm on a number of simple synthetic spatiotemporally-evolving systems. One is displayed in Figure \ref{fig:synthetic}), having two invariant subspaces, each of which has two hills oscillating up and down in sync at a different frequency. Our method is able to successfully divide the system nicely through the middle, as expected (Figures \ref{fig:init_cluster}, \ref{fig:final_cluster}).

We also tested our algorithm on fluid flow data sets (see Sidebar \ref{sb:cfd}). We found that our algorithm was able to separate divide different parts of the flow in a way that makes sense, see Figures \ref{fig:init_clusterCFD},\ref{fig:final_clusterCFD}.
...
In order to verify empirically our theoretical results, we compared the eigenvalues and Koopman modes derived from $\dualopApprox$ in our E-GP model with the eigenvalues and Koopman modes derived by the well-known Dynamic Mode Decomposition (DMD) algorithm using data from fluid flowing past a cylinder at different Reynolds numbers. The results are presented in figures \ref{fig:cfd_100_modes} and \ref{fig:eigenvalues}.
